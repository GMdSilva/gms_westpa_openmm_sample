...
class WEDrudeProd:
    ...
    def build_simulation_obj(self):
        self.integrator.setRandomNumberSeed(1)
        platform = Platform.getPlatformByName(self.configs['platform'])

        self.simulation = Simulation(self.psf_box.topology, self.system, self.integrator, platform)
    ...
    def run_prod(self):
        print(f"Reading parameters for OpenMM simulation with DrudeFF at {self.configs['base_dir']}\n")
